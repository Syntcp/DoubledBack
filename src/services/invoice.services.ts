...
export async function generateInvoicePdf(userId: number, id: number): Promise<Buffer> {
  ...
  return await new Promise<Buffer>(async (resolve, reject) => {
    ...
    const cols = [
      { label: 'Description', w: Math.floor(contentW * 0.5), align: 'left' as const },
    ...
    const drawNotes = async () => {
      ...
      try {
        const ctx = await getVatContext(Number(inv.ownerId), inv.issueDate);
